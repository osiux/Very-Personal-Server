...
class TransmissionRPC
{
  ...
  {
    if ( !is_array( $ids ) ) $ids = array( $ids );	// Convert $ids to an array if only a single id was passed
    if ( count( $fields ) == 0 ) $fields = array( "id", "name", "status", "doneDate", "haveValid", "totalSize", "sizeWhenDone", "percentDone", "addedDate", "rateDownload", "rateUpload", "status", "leftUntilDone", "desiredAvailable" );	// Defaults
    $request = array(
      "fields" => $fields,
      "ids" => $ids
    );
...
*/
  ...
  public function sinfo($request=array())
  {
...
/**
* Getting Session Info
* @param array options A list of parameters and values to set
* see: https://trac.transmissionbt.com/browser/trunk/extras/rpc-spec.txt  for a list of available parameters
*/

  public function sset($options=array())
  {
    return $this->request( "session-set", $options);
  }
   ...
   */
  public function __construct($url=null, $username = null, $password = null, $return_as_array = false )
  {
    // server URL
    $this->url = $url;
